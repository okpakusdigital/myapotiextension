...
window.MyApotiUtils = {
  ...
  async sendToMyApoti(endpoint, payload) {
...
window.MyApotiUtils = {
  ...
  async sendToMyApoti(endpoint, payload) {
    ...
    try {
      ...
      // ── RetailMan is active — extension blocked ──
      // Store the blocked state so the popup can show a clear message.
      if (res.status === 409) {
        const err = await res.json().catch(() => ({}));
        const detail = err.detail || "RetailMan sync is active — extension disabled.";
        console.warn("MyApoti: Extension blocked —", detail);
        chrome.storage.local.set({
